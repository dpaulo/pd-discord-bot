...
module.exports = async (message) => {
  ...
  if (args.length !== 2) {
    ...
    try {
      ...
      return message.channel.send(`The current price of 1 ${coin} = ${data[coin][vsCurrency]} ${vsCurrency}`);
    } catch (err) {
      return message.channel.send("Please check your inputs. For example: $coinprice bitcoin usd");
    }
  }
};
